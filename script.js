...
function sendMessage() {
	...
	if (!socket) {
		alert("RelaxChat is not connected to the server yet.");
		return;
	}
...
if (socket) {
	...
	socket.on("newMessage", function(message) {
		if (!allMessages[message.channel]) allMessages[message.channel] = [];
	...
	socket.on("connect_error", function(error) {
		...
		chatSubtitle.textContent = "Could not connect to server.";
	});
} else {
	chatSubtitle.textContent = "Socket.IO did not load.";
}
