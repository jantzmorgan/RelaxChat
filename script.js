document.addEventListener("DOMContentLoaded", function() {
	...
	let ownedRoomIds = new Set();
	let roomNameOverrides = JSON.parse(localStorage.getItem("relaxChatRoomNameOverrides") || "{}");
	let messages = [];
	let allMessages = {};
	let sounds = [];
	let allSounds = [];
	...
	function canRenameCurrentRoom() {
		...
		return isAdminUser() || ownedRoomIds.has(currentRoom);
	}

	function getSavedRoomName(roomId, fallbackName) {
		if (roomNameOverrides[roomId]) {
			return roomNameOverrides[roomId];
		}
		...
		return fallbackName || "RelaxChat";
	}

	function updateChatNameEditAccess() {
		if (!chatNameInput) return;

		const canEdit = canRenameCurrentRoom();

		chatNameInput.readOnly = !canEdit;
		chatNameInput.tabIndex = canEdit ? 0 : -1;
		chatNameInput.inputMode = canEdit ? "text" : "none";
		chatNameInput.setAttribute("aria-readonly", String(!canEdit));
		chatNameInput.classList.toggle("canRenameChat", canEdit);
	...
	function setScreenViewerMirror(shouldMirror) {
		...
		screenViewer.style.transform = shouldMirror ? "scaleX(-1)" : "";
		screenViewer.style.transformOrigin = "center center";
	}

	function setCameraViewerMode(shouldUseCameraMode) {
		if (!screenViewer) return;

		const viewerBox = screenViewer.closest(".mobileStreamViewerBox");

		screenViewer.classList.toggle("snapCameraView", Boolean(shouldUseCameraMode));

		if (shouldUseCameraMode) {
			screenViewer.removeAttribute("controls");
		} else {
			screenViewer.setAttribute("controls", "");
		}

		if (viewerBox) {
			viewerBox.classList.toggle("cameraMode", Boolean(shouldUseCameraMode));
		}
		if (cameraFullscreenBtn) {
			cameraFullscreenBtn.classList.toggle("hidden", !shouldUseCameraMode);
		}
	}

	function setCameraFullscreenMode(shouldUseFullscreen) {
		if (!screenViewer) return;

		const viewerBox = screenViewer.closest(".mobileStreamViewerBox");
		if (!viewerBox) return;

		viewerBox.classList.toggle("cameraFullscreen", Boolean(shouldUseFullscreen));

		if (cameraFullscreenBtn) {
			cameraFullscreenBtn.textContent = shouldUseFullscreen ? "Exit Full Screen" : "Full Screen";
		}
	}

	function toggleCameraFullscreen() {
		const viewerBox = screenViewer ? screenViewer.closest(".mobileStreamViewerBox") : null;
		if (!viewerBox) return;

		const isFullscreen = viewerBox.classList.contains("cameraFullscreen");
		setCameraFullscreenMode(!isFullscreen);
	}
	...
	async function loadRooms() {
		...
		result.rooms.forEach(function(room) {
			if (currentUser && room.ownerId === currentUser.id) {
				ownedRoomIds.add(room.id);
			}

			const displayRoomName = getSavedRoomName(room.id, room.name);
			...
			roomBtn.textContent = displayRoomName;

			roomBtn.addEventListener("click", function() {
				switchRoom(room.id, displayRoomName);
			});
	...
	function switchRoom(roomId, roomName) {
		...
		currentRoom = roomId;
		currentRoomName = getSavedRoomName(roomId, roomName);
	...
	function updateStreamSupport() {
		...
		if (isScreenStreaming) {
			...
			if (currentStreamType === "camera") {
				if (cameraFullscreenBtn) {
					cameraFullscreenBtn.classList.remove("hidden");
				}

				if (streamSupportNote) {
					streamSupportNote.textContent = "Your camera is already live. Use Stop Stream to end it.";
				}
			}
	...
	function showOwnStreamPreview() {
		...
		setCameraViewerMode(localScreenStreamIsSelfie);

		if (cameraFullscreenBtn && currentStreamType === "camera") {
			cameraFullscreenBtn.classList.remove("hidden");
		}
	...
	function renderStreamPlaceholder() {
		setCameraViewerMode(false);
		setScreenViewerMirror(false);
		if (screenViewer) {
			screenViewer.pause();
	...
	async function createScreenPeerConnection(peerSocketId, isBroadcaster, streamId) {
		...
		peer.ontrack = function(event) {
			...
			const shouldMirrorWatchedStream = watchedStream && watchedStream.isSelfieCamera;

			setCameraViewerMode(shouldMirrorWatchedStream);
			setScreenViewerMirror(shouldMirrorWatchedStream);
			screenViewer.style.transform = "";
	...
	chatNameInput.addEventListener("focus", updateChatNameEditAccess);
	chatNameInput.addEventListener("click", updateChatNameEditAccess);

	chatNameInput.addEventListener("input", function() {
		if (!canRenameCurrentRoom()) {
			chatNameInput.value = currentRoomName || "RelaxChat";
			return;
		}

		currentRoomName = chatNameInput.value.trim() || "RelaxChat";

		roomNameOverrides[currentRoom] = currentRoomName;
		localStorage.setItem("relaxChatRoomNameOverrides", JSON.stringify(roomNameOverrides));
		localStorage.setItem("relaxChatCurrentRoomName", currentRoomName);
	...
	if (cameraFullscreenBtn) {
		cameraFullscreenBtn.addEventListener("click", toggleCameraFullscreen);
	}

	if (screenViewer) {
		screenViewer.addEventListener("click", function() {
			const viewerBox = screenViewer.closest(".mobileStreamViewerBox");

			if (viewerBox && viewerBox.classList.contains("cameraFullscreen")) {
