document.addEventListener("DOMContentLoaded", function() {
	...
	function updateStreamSupport() {
		const canStartScreenStream = !!(
			navigator.mediaDevices &&
			navigator.mediaDevices.getDisplayMedia
		);

		if (!canStartScreenStream) {
			if (startStreamBtn) {
				startStreamBtn.classList.add("hidden");
			}

			if (streamSupportNote) {
				streamSupportNote.textContent = "This device can watch streams, but this browser does not support starting a screen stream.";
			}
		} else if (streamSupportNote) {
			streamSupportNote.textContent = "This device may be able to start a screen stream. Watching streams should work.";
		}
	}
	...
	function showOwnStreamPreview() {
		...
		setStreamFocusStatus("Previewing your own stream. Others can still watch you.");
	}
	...
	async function startScreenStream() {
		...
		if (!navigator.mediaDevices || !navigator.mediaDevices.getDisplayMedia) {
			alert("This browser cannot start screen sharing. You should still be able to watch streams.");
			updateStreamSupport();
			return;
		}
	...
	function stopScreenStream() {
		...
		if (startStreamBtn && navigator.mediaDevices && navigator.mediaDevices.getDisplayMedia) {
	...
	if (socket) {
			...
			localScreenStream.getTracks().forEach(function(track) {
				...
				if (track.kind === "video") {
					...
					sender.setParameters(parameters).catch(function(error) {
						console.log("Could not set screen bitrate:", error);
					});
				}
			});
		...
		socket.on("screenStreamUnavailable", function() {
			alert("That stream is no longer available.");
			stopWatchingScreenStream();
		});
	...
	if (stopStreamBtn) {
		stopStreamBtn.addEventListener("click", stopScreenStream);
	}
